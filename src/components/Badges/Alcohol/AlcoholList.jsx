...
const AlcoholList = ({ userBadges }) => {
  return (
    <div id="badgesAlcohol" className={`flex flex-col rounded-lg border-[#7f1734] border xs:w-[90%] w-[95%]`}>
      <div id="badgesAlcoholTitle" className={`flex bg-[#7f1734] rounded-t-md min-w-[90%] text-[24px]`}>
      ...
      <div id="badgesAlcoholList" className={`min-w-[90%]`}>
        <div className={`flex flex-col`}>
          {badges
            .filter((badge) => badge.group === "alcohol")
            .sort((badge1, badge2) => badge2.update - badge1.update)
            .map((badge, index) => (
              <div className={`flex w-full ${index === badges.filter((badge) => badge.group === "alcohol").length - 1 ? "" : "border border-transparent border-b-[#7f1734]"}`}>
