...
const JuegosDeMesaList = ({ userBadges }) => {
  return (
    <div id="badgesJuegosDeMesa" className={`flex flex-col rounded-lg border-white border xs:w-[90%] w-[95%]`}>
      <div id="badgesJuegosDeMesaTitle" className={`flex bg-white rounded-t-md min-w-[90%] text-[24px] text-black`}>
      ...
      <div id="badgesJuegosDeMesaList" className={`min-w-[90%]`}>
        <div className={`flex flex-col`}>
          {badges
            .filter((badge) => badge.group === "juegos_de_mesa")
            .sort((badge1, badge2) => badge2.update - badge1.update)
            .map((badge, index) => (
              <div className={`flex w-full ${index === badges.filter((badge) => badge.group === "juegos_de_mesa").length - 1 ? "" : "border border-transparent border-b-white"}`}>
...
export default JuegosDeMesaList