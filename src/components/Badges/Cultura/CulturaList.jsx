...
const CulturaList = ({ userBadges }) => {
  return (
    <div id="badgesCultura" className={`flex flex-col rounded-lg border-blue-300 border xs:w-[90%] w-[95%]`}>
      <div id="badgesCulturaTitle" className={`flex bg-blue-300 rounded-t-md min-w-[90%] text-[24px]`}>
      ...
      <div id="badgesCulturaList" className={`min-w-[90%]`}>
        <div className={`flex flex-col`}>
          {badges
            .filter((badge) => badge.group === "cultura")
            .sort((badge1, badge2) => badge2.update - badge1.update)
            .map((badge, index) => (
              <div className={`flex w-full ${index === badges.filter((badge) => badge.group === "cultura").length - 1 ? "" : "border border-transparent border-b-blue-300"}`}>
