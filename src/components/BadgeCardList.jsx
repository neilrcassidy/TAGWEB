...
const BadgeCardList = ({ userBadges, title, category, emoji, color, newCategory }) => {
  return (
    <div id={"badges"+title} className={`flex flex-col rounded-lg ${"border-"+color} border xs:w-[90%] w-[95%]`}>
      <div id={"badges"+title+"Title"} className={`flex ${"bg-"+color} rounded-t-md min-w-[90%] text-[24px] ${color === "white" ? "text-black" : ""}`}>
      ...
      <div id={"badges"+category+"List"} className={`min-w-[90%]`}>
        <div className={`flex flex-col`}>
          {badges
            ...
            .map((badge, index) => (
              <div className={`flex w-full ${index === badges.filter((badge) => badge.group === category).length-1 ? "" : "border border-transparent border-b-" + color}`}>
                <div className={`flex w-full my-4 ml-2`}>
                  <div className={`flex ss:min-w-[96px] ss:w-[96px] min-w-[64px] w-[64px] m-auto mr-2 ss:ml-2 ml-0 `}>
                    <div className={`relative`}>
                      ...
                      {badge.update===1 ? <img className={`absolute top-[62px] left-[62px] w-[40px]`} src={update1}></img> : ""}
                    </div>
                  </div>
